...
test('verify_completion returns a structured error instead of throwing when the mandate has no escrow entry', () => {
  // Deviation from the brief (documented in task-15-report.md): a mandate that exists but was
  // never committed through commit_mandate (so openEscrow was never called for it) would, under
  // the brief's literal code, pass the `!mandate` guard and then crash uncaught inside
